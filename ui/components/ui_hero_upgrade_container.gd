...
var _hero_stat_change_scene: PackedScene = preload("res://ui/components/ui_hero_stat_change_container.tscn")
...
func initialize(upgrade: SurvivalHeroUpgrade):
	...
	if upgrade.stats_modifier.max_hp != 0:
		var stat_change: UIHeroStatChangeContainer = _hero_stat_change_scene.instantiate()
